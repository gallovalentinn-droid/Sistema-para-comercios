...
test('el paquete incluye verificación de integridad y la ruta anterior ya no carga otro sistema', () => {
  ...
  assert.equal(manifest.packageRevision, 43);
...
test('la identidad de la corrección queda alineada en REV43', () => {
  const text = source();
  const sw = fs.readFileSync(swPath, 'utf8');
  assert.match(text, /packageRevision:43/);
  assert.match(sw, /micomercio-beta-6\.0\.0-f6-rc2-rev43/);
});
