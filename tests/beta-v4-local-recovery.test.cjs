...
function load({ localProducts = [], localSales = [], remoteProducts = 660, remoteSales = 63 } = {}) {
  const cursor = { ts: '2026-09-24T20:30:00Z', id: '11111111-1111-4111-8111-111111111111' };
  const state = {
    cursores: { productos: { ...cursor }, ventas: { ...cursor } },
    diferidos: { productos: {}, clientes: {}, combos: {}, promociones: {}, config: null },
    stats: {},
  };
  const context = {
    db: { productos: localProducts, ventas: localSales },
    ...
    sb: {
      from(table) {
        return {
          ...
          eq() { return this; },
          is() { return this; },
          then(resolve, reject) { return Promise.resolve({ count: table === 'productos' ? remoteProducts : remoteSales, error: null }).then(resolve, reject); },
        };
      },
    },
  };
