# Mi Comercio — QA F4.3 bootstrap v0.1.1
...
## Orden canónico de reconstrucción
...
5. F3.2 índices slice A/B vigentes.
6. `supabase-v4-f3.3-contracts.sql` — F3.3.
7. `supabase-v4-f3.4-contracts.sql` — F3.4.
8. `supabase-v4-f4.1-control-plane.sql` — F4.1.
9. `supabase-v4-f4.2-zero-start.sql` — F4.2.
...
2. Colocar los instaladores canónicos exactos dentro de `canonical_sources/`.
3. Ejecutar `python scripts/check_canonical_files.py canonical_sources`.
4. Si el checker devuelve `READY_FOR_SANDBOX_REBUILD=false`, no resetear nada.
5. Con todos los artefactos presentes, reconstruir el sandbox.
6. Ejecutar `qa/01_parity_fingerprint.sql` en el sandbox.
7. Exigir que **todos** los hashes/counts coincidan con `EXPECTED_PROD_FINGERPRINT.json`.
8. Ejecutar `qa/02_required_contracts.sql`.
9. Ejecutar `qa/03_extra_object_audit.sql`.
10. Recién con PARIDAD PASS, preparar/ejecutar COV-1.
11. Después: multi-tenant/RLS → rollback F4.3 → regresión → candidatear F4.3.
