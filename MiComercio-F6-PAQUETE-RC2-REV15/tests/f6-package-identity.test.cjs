...
test('la identidad normativa F6 fija contratos, conteos y hash del artefacto', () => {
  ...
  assert.equal(identity.package_revision, 15);
  assert.equal(identity.expected_local_tests, 156);
  ...
  assert.equal(path.basename(ROOT), 'MiComercio-F6-PAQUETE-RC2-REV15');
