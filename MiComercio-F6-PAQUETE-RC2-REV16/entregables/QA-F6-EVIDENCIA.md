...
## RC2 revisión 6 — lector de facturas con IA — 2026-09-08
...
- La suite prueba permisos de dueño y empleado, veto a empleado sin Productos, licencia vencida, idempotencia y frontera exacta del cupo mensual de 100 lecturas por comercio. Las filas del mes anterior no consumen el cupo vigente.
...
## Corrección RC2 revisión 14 — acceso de empleados — 2026-09-14

- El diagnóstico confirmó que `f5-login` ya implementaba del lado servidor el acceso por código de comercio, usuario y contraseña, pero el HTML público sólo ofrecía el ingreso directo por correo del dueño.
- La pantalla inicial separa ahora `Dueño o administrador` de `Empleado`. El flujo de empleado llama a `/functions/v1/f5-login`, instala los tokens devueltos con `auth.setSession` y después atraviesa los controles existentes de contexto V4, membresía, licencia y permisos.
- El cliente no crea, envía ni muestra el correo técnico interno. Las invitaciones continúan forzando el flujo de dueño para no mezclar el alta con el ingreso del personal.
- Los errores locales conservan un código estable para mostrar mensajes útiles; una respuesta 401 sigue siendo genérica y un 429 informa la espera de quince minutos.
- La batería local completa pasó 148/148. La suite pública pasó 22/22 antes del despliegue.
- GitHub Pages publicó `packageRevision: 14` desde los commits `eb95576` y `3a0386c`. Una lectura independiente del dominio devolvió HTTP 200 y confirmó la revisión, el enlace a `f5-login` y la corrección de validación.
- Falta una prueba operativa con credenciales de un empleado real del comercio piloto; no se fabricó ni modificó una cuenta para simularla.

## Corrección RC2 revisión 15 — compuerta de onboarding en el acceso de empleados — 2026-09-15

Origen: revisión externa del paquete REV14. La revisión 14 dejó registrado que faltaba
"una prueba operativa con credenciales de un empleado real"; el defecto que sigue vive
exactamente en el tramo que esa prueba no cubrió.

- Defecto: todo login de empleado terminaba llamando `f6OnboardingPendiente()`, que consulta
  `f6_onboarding_actual`. Esa función ejecuta `private._f6_assert_owner` de forma incondicional
  (`supabase/f6/03_onboarding.sql`) y levanta `F6_ONBOARDING_OWNER_REQUIRED` para cualquier
  miembro que no sea `duenio`. El cliente propagaba la excepción y el ingreso del empleado
  fallaba, con licencia en caché operable mediante una pantalla muerta y sin ella con el
  mensaje genérico de conexión.
- Reproducción: ejecutar `f6OnboardingPendiente` con un cliente de onboarding que levante
  `F6_ONBOARDING_OWNER_REQUIRED`. En REV14 lanza; en REV15 devuelve `false`.
- Corrección 1: `f6OnboardingPendiente` reconoce ese veto y devuelve `false` en lugar de
  propagarlo. Cualquier otro error del onboarding se sigue propagando sin cambios. El núcleo
  quedó delimitado por `F6_ONBOARDING_GATE_CORE_START/END` para poder probarlo aislado.
- Corrección 2: el submit de `#formLogin` envuelve `f43PostLoginSuccess()` en `try/catch`.
  Antes, una falla al abrir el comercio terminaba en una promesa rechazada sin manejar y la
  pantalla quedaba sin aplicación y sin mensaje.
- Regresión: `tests/f6-onboarding-gate-empleado.test.cjs`, ocho casos. Cubre el veto de dueño,
  la propagación de otros errores, el asistente del dueño con onboarding incompleto y completo,
  el corto circuito sin comercio resuelto, el reconocimiento del error por message/code/details/hint
  y la presencia del `try/catch` en el submit. La batería local sube a 156.
- No se modificó ningún archivo de `supabase/`: el contrato SQL ya era correcto y el veto de
  dueño sobre el onboarding se conserva intacto.
- Pendiente sin cambios: la prueba operativa con un empleado real del comercio piloto sigue
  siendo necesaria. Esta corrección la habilita; no la reemplaza.

## Corrección RC2 revisión 16 — autoridad en el primer arranque del empleado — 2026-09-15

- La prueba operativa de REV15 alcanzó el servidor, pero el primer chequeo de autoridad ocurrió
  antes del alta del dispositivo. El evento remoto registró `F5_DEVICE_REVOKED`; después el alta
  quedó activa, pero el cliente ya había continuado sin lease y mostró modo sólo lectura.
- Las consultas de diagnóstico confirmaron membresía `empleado` activa, versión de permisos 2,
  licencia operable hasta 2099, proyección 19/19 y lecturas RLS correctas. No se modificaron esos
  contratos ni se aplicaron migraciones.
- `f42PrepararF3ParaPull()` registra ahora el dispositivo antes del chequeo, solicita el lease con
  `emitir:true` y exige `writable:true` antes de activar el contexto de pull.
- `tests/f6-employee-startup-authority.test.cjs` reproduce los dos defectos sobre el código real:
  el orden anterior y la continuación indebida frente a una denegación. Ambos fallaban en REV15
  y pasan en REV16. La batería declarada sube a 158/158.
- Queda como gate operativo recargar la beta publicada e ingresar nuevamente con el empleado
  real. No hace falta cambiar su contraseña, membresía, permisos ni licencia.
