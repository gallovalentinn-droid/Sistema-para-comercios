...
La RPC operativa acepta sólo `fondo_caja`, `fondo_caja_cigarros`, `dias_aviso_vence`, `dias_plazo_fiado`, `recargo_fiado_pct`, `motivos_egreso_extra`. La privilegiada acepta sólo `modulo_fiado`, `modulo_vencimientos`, `modulo_cigarros` y `whatsapp_dueno`, y exige dueño/admin. `pin_hash` y `permisos_empleado` quedan legacy-only: no tienen escritor autoritativo en V4 y no forman parte de ninguna de las dos RPC.
