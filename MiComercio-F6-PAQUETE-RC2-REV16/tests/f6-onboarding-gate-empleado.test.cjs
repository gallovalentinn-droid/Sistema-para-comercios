...
const artifactPath = path.join(ROOT, 'entregables', 'MiComercio-F6-PRUEBA.html');
const onboardingSqlPath = path.join(ROOT, 'supabase', 'f6', '03_onboarding.sql');
...
test('el onboarding es exclusivo del dueño también en el contrato SQL', () => {
  const sql = fs.readFileSync(onboardingSqlPath, 'utf8');
  // f6_onboarding_actual valida propiedad SIEMPRE, no sólo cuando falta completar.
  assert.match(sql, /create or replace function public\.f6_onboarding_actual[\s\S]*?_f6_assert_owner/i);
  assert.match(sql, /F6_ONBOARDING_OWNER_REQUIRED/);
});
