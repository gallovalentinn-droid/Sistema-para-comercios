...
test('el contrato elimina getPublicUrl y vuelve privado el bucket', () => {
  ...
  const migration = fs.readFileSync(path.resolve(__dirname, '../supabase/f6/11_product_images_private.sql'), 'utf8');
  const sqlTest = fs.readFileSync(path.resolve(__dirname, '../supabase/tests/f6_employees_images.test.sql'), 'utf8');

  assert.doesNotMatch(core, /getPublicUrl/u);
  assert.match(core, /createSignedUrls/u);
  assert.match(migration, /update\s+storage\.buckets[\s\S]*set\s+public\s*=\s*false[\s\S]*id\s*=\s*'product-images'/iu);
  assert.match(sqlTest, /id\s*=\s*'product-images'[\s\S]*and\s+not\s+public/iu);
});
